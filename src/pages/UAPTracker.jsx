...
import { Stars, OrbitControls, Text, DeviceOrientationControls, Billboard } from '@react-three/drei'
...
function Satellites({ observerLat, observerLon, showLabels, azimuthOffset, filterVisible }) {
    ...
    useFrame(() => {
        ...
        const positions = satellites.map(sat => {
            ...
            // Atmospheric Fade (Elevation Opacity)
            // Fade out between 0 and 15 degrees elevation
            const elevationDeg = relativePos.elevation * (180/Math.PI);
            const opacity = Math.min(1, Math.max(0, elevationDeg / 15));
            ...
            {satPositions.map((sat, idx) => (
                <group key={idx} position={sat.position}>
                    <mesh>
                        ...
                        <meshBasicMaterial color="#00ff00" transparent opacity={sat.opacity} />
                    </mesh>
                    {showLabels && (
                        <Billboard>
                             <Text
...
function Planes({ observerLat, observerLon, showLabels, azimuthOffset }) {
    ...
    useFrame(() => {
        ...
        const positions = planes.map(plane => {
            ...
            // Atmospheric Fade
            const elevationDeg = relativePos.elevation * (180/Math.PI);
            const opacity = Math.min(1, Math.max(0, elevationDeg / 10));
            ...
            {planePositions.map((plane, idx) => (
                <group key={idx} position={plane.position}>
                    <mesh>
                        ...
                        <meshBasicMaterial color="#00ffff" transparent opacity={plane.opacity} />
                    </mesh>
                    {showLabels && (
                        <Billboard>
                            <Text
                                ...
                                anchorX="center"
                                anchorY="top"
                                fillOpacity={plane.opacity}
                                outlineOpacity={plane.opacity}
                            >
                                {plane.callsign}
                            </Text>
                        </Billboard>
    ...
    return (
        <>
            ...
            <SkyGroup azimuthOffset={azimuthOffset}>
                ...
                {/* Fixed orientation Text facing South (rotationY = PI) so it's readable from origin */}
                <Text
                    position={[0, -2, -50]}
                    fontSize={5}
                    color="red"
