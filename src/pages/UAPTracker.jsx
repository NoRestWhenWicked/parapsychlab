...
function Satellites({ observerLat, observerLon, showLabels, azimuthOffset }) {
...
// Scene Wrapper to handle rotation group based on offset?
// Actually, it's better to pass offset to calculation or rotate the whole group.
// Rotating the whole group is cheaper.
function SkyGroup({ children, azimuthOffset }) {
    const groupRef = useRef();
    useFrame(() => {
        if (groupRef.current) {
            // Rotate around Y axis.
            // Azimuth is usually clockwise from North.
            // If we have an offset (e.g. +10 deg), we should rotate the world -10 deg or +10 deg?
            // If the compass reads North but it's actually North-East, we need to correct.
            // Let's assume offset adds to the azimuth calculation.
            // Actually, rotating the group is better.
            // If offset is positive, it means we shift everything East?
...
function Scene({ arMode, showLabels, observerLat, observerLon, azimuthOffset }) {
    return (
        <>
            ...
            <SkyGroup azimuthOffset={azimuthOffset}>
                ...
                <Satellites observerLat={observerLat} observerLon={observerLon} showLabels={showLabels} azimuthOffset={0} />
